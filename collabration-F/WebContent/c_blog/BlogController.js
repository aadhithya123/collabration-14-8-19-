myApp.controller("blogController",function($scope,$location,$rootScope,$http,$cookieStore)
	...
	$scope.useradmin={"userid":"","username":"","password":"","emailid":"","useraddress":"","userphone":"","role":"","status":"","isonline":""};
...
//		});
	...
	var url=''
	   ...
	   $scope.useradmin=$cookieStore.get("userDetails")
	   if($scope.useradmin.role=='ROLE_ADMIN')
		   {
		   url='http://localhost:9092/ConnectPeopleMiddle1/showAllBlogForApproval'
		   }
	   else
		   {
		   url='http://localhost:9092/ConnectPeopleMiddle1/showAllBlog'
		   }
	$http.get(url)
	.then(function(response)
	{
